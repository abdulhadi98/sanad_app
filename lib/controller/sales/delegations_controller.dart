  ...
  getDelegations() async {
    ...
    try {
      dynamic response = await http.get(
          Uri.parse(UrlsContainer.getDelegationsOrders),
          headers: {'Authorization': 'Bearer $token'});
      dynamic body = jsonDecode(response.body);
      print(body);
      List<dynamic> data = body['data'];
      delegationsList = List<DelegationModel>.from(
          data.map((x) => DelegationModel.fromJson(x)).toList());
      delegationsList
          .removeWhere((delegation) => delegation.employeeId != null);
